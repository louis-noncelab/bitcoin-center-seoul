...
The selected filter background glides between buttons using the existing shared-layout mechanism, 240ms control duration and ease-out curve. Labels and button geometry remain stationary, including wrapped mobile rows. A newly selected list fades in over the same duration; selection updates immediately, without queued exits or duplicate actionable rows. Re-selecting the current filter or updating sold-out state does not restart the list animation. Reduced motion settles both effects immediately, including preference changes during use. Mechanism reference: https://beui.dev/r/tabs/raw; reuse installed Motion and CSS without new dependencies.

AdminTabs shares this selection motion between collection filters and event/highlight tabs. All four admin workspaces use the same short opacity entrance for headings, toolbars, loaded lists and registration/edit forms; keep the header and current controls stationary. Re-selecting the active event/highlight list is a no-op, while selecting from an editor retains the existing unsaved-change confirmation. Navigation, authentication, fetch cancellation, registration/sold-out switches and in-progress form data remain unchanged; no delayed navigation, stale outgoing controls or timer-driven state changes.

The public collection's URL-based filters also fade the newly selected grid or empty state over the control duration. Preserve real links, browser history, server rendering and bilingual labels. Reduced motion skips the fade, matching the existing public review filters.
