...
- Native text, password, URL, date, file, select and textarea controls share a `FormControl` wrapper. Keep native semantics, labels, validation, autocomplete, file selection and textarea resizing. Textareas retain their requested rows; hidden, checkbox and radio inputs use their existing native controls.
...
- One-line fields and ordinary buttons share the existing 48px minimum control height, 16px text, 1.5 line height, 8px corners and border-aware padding. Multiline buttons may grow for text zoom. Native selects retain their native picker; no new selection feature is introduced.
