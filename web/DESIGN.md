...
- Home's Program details link sits on the right of the education/meetup tab row, next to its bottom divider, using the same 48px interaction height. The heading remains above. On narrow screens both controls stay aligned without overlap.
...
- Header operating status sits immediately before the language control: a space-2 dot and text, without another filled pill. States are `운영 중 / Open`, `행사 진행 중 / Event on`, and `운영 종료 / Closed`; dot colors reuse success, orange and muted. Text stays ink/muted for contrast. Compact mobile labels can wrap at spaces inside a 64px target; all controls retain 48px height. Collapse desktop navigation before controls collide.
- Status authority defaults to a manual choice in the existing authenticated admin workspace, valid only for the date selected in Asia/Seoul. Unset, expired or unavailable data shows `운영 안내 / Visit info`, linking to visit information without claiming a real-time open state. Refresh while visible every minute and on visibility/focus; never infer holidays or private rentals from usual hours.
