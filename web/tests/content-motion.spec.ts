...
test("outgoing space content stays inert during a tab transition", async ({ page }) => {
  // Given a space selector with enough transition time to inspect the overlap.
  ...
  // When the visitor selects the next space.
  await tabs.nth(1).click();

  // Then both plates can crossfade, but only the selected space is accessible.
...
test("repeated selection settles immediately when reduced motion changes", async ({ page }) => {
  ...
  await page.emulateMedia({ reducedMotion: "no-preference" });
  await page.goto("/en/about#space-tour");
  await page.addStyleTag({ content: ".selection-panel { --duration-control: 10s; --duration-image: 10s; }" });
  const tabs = page.getByRole("tab");
  await tabs.nth(1).click();
  ...
  await expect.poll(() => active.locator("video").evaluate((video: HTMLVideoElement) => video.paused)).toBe(true);
...
test("server-rendered content remains visible without JavaScript", async ({ browser }) => {
  ...
  try {
    ...
    // Then the photograph and discovery sections are available without hydration.
