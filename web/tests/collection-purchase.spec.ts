...
for (const kind of ["book", "goods"] as const) {
  test(`${kind}: admin purchase links, public routing, locales and removal`, async ({ page, baseURL }) => {
    ...
    const purchaseUrl = "https://pay.example.com/bcs?item=book&ticket=1";
    ...
    try {
      ...
      await page.getByLabel("소개 (선택)", { exact: true }).fill("센터에서 만나는 비트코인. 구매하기 버튼에서 외부 판매 페이지를 확인하세요.");
      await page.getByLabel("구매하기 링크 (선택)", { exact: true }).fill(purchaseUrl);
      ...
      id = item.id;
      expect(item.purchaseUrl).toBe(purchaseUrl);
      ...
      expect((await page.request.put(`/api/admin/collection/${id}`, { headers: { origin, "If-Match": `"${revision}"` }, data: { ...input, purchaseUrl: "javascript:alert(1)" } })).status()).toBe(400);
      expect((await page.request.get(`/ko/${kind === "goods" ? "collection" : "goods"}/${slug}`)).status()).toBe(404);
      expect(await (await page.request.get("/sitemap.xml")).text()).toContain(`/${path}/${slug}</loc>`);
      for (const locale of ["ko", "en"] as const) {
        for (const width of [1440, 390]) {
          ...
          const buy = card.getByRole("link", { name: locale === "ko" ? "구매하기 (새 창)" : "Buy (new window)", exact: true });
          await expect(buy).toHaveAttribute("href", purchaseUrl);
          await expect(buy).toHaveAttribute("target", "_blank");
          await expect(buy).toHaveAttribute("rel", "noopener noreferrer");
          // Intercept the destination so this check never contacts a payment service.
          await page.context().route("https://pay.example.com/**", route => route.fulfill({ body: "External purchase destination" }));
          if (width === 1440 && locale === "ko") {
            const popupEvent = page.waitForEvent("popup");
            await buy.click();
            const popup = await popupEvent;
            await popup.waitForLoadState();
            expect(popup.url()).toBe(purchaseUrl);
          ...
          await expect(page.locator("main").getByRole("link", { name: locale === "ko" ? "구매하기 (새 창)" : "Buy (new window)", exact: true })).toHaveAttribute("href", purchaseUrl);
          await expect(page.locator('link[rel="canonical"]')).toHaveAttribute("href", new RegExp(`/${locale}/${path}/${slug}$`));
          expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
          await capture(page, { path: `${evidence}/${kind}-${locale}-${width}-detail.png`, fullPage: true });
        }
      }
      await page.goto("/ko/admin/collection");
      const toggle = page.getByRole("switch", { name: `${title} 품절`, exact: true });
      await expect(toggle).toHaveAttribute("aria-checked", "false");
      await toggle.click();
      await expect(toggle).toHaveAttribute("aria-checked", "true");
      await toggle.scrollIntoViewIfNeeded();
      await capture(page, { path: `${evidence}/${kind}-admin-sold-out.png` });
      const updated = recordsSchema.parse(await (await page.request.get("/api/admin/collection")).json()).data.find(record => record.id === id);
      expect(updated).toMatchObject({ purchaseUrl, soldOut: true, revision: revision + 1 });
      expect((await page.request.patch(`/api/admin/collection/${id}`, { headers: { origin, "If-Match": `"${revision}"` }, data: { soldOut: false } })).status()).toBe(409);
      for (const locale of ["ko", "en"] as const) {
        await page.goto(`/${locale}/${path}`);
        const card = page.locator(".review-card").filter({ hasText: title });
        await expect(card.locator(".collection-sold-out")).toHaveText(locale === "ko" ? "품절" : "Sold out");
        await expect(card.locator("a[target='_blank']")).toHaveCount(0);
        await page.goto(`/${locale}/${path}/${slug}`);
        await expect(page.locator(".collection-sold-out")).toHaveText(locale === "ko" ? "품절" : "Sold out");
        await expect(page.locator(".collection-sold-out")).toHaveAttribute("aria-disabled", "true");
        await capture(page, { path: `${evidence}/${kind}-${locale}-sold-out.png`, fullPage: true });
      }
      await page.goto("/ko/admin/collection");
      ...
      await expect(page.getByLabel("구매하기 링크 (선택)", { exact: true })).toHaveValue(purchaseUrl);
      await page.getByLabel("구매하기 링크 (선택)", { exact: true }).scrollIntoViewIfNeeded();
      expect(await page.locator('.events-checkbox:has(input[name="soldOut"])').evaluate(label => {
        const text = [...label.childNodes].find(node => node.nodeType === Node.TEXT_NODE && node.textContent?.trim() === "품절");
        if (!text) return 0;
        const range = document.createRange();
        range.selectNodeContents(text);
        return range.getClientRects().length;
      })).toBe(1);
      await capture(page, { path: `${evidence}/${kind}-admin-mobile.png`, fullPage: true });
      await page.setViewportSize({ width: 1440, height: 900 });
      await capture(page, { path: `${evidence}/${kind}-admin-desktop.png`, fullPage: true });
      await page.locator('input[name="soldOut"]').uncheck();
      await page.getByRole("button", { name: "저장", exact: true }).click();
      await expect(page.getByText("저장했습니다.", { exact: true })).toBeVisible();
      await page.goto(`/ko/${path}/${slug}`);
      await expect(page.getByRole("link", { name: "구매하기 (새 창)", exact: true })).toHaveAttribute("href", purchaseUrl);
      await page.goto("/ko/admin/collection");
      await page.getByRole("switch", { name: `${title} 품절`, exact: true }).click();
      await expect(page.getByRole("switch", { name: `${title} 품절`, exact: true })).toHaveAttribute("aria-checked", "true");
      await page.getByRole("switch", { name: `${title} 품절`, exact: true }).click();
      await expect(page.getByRole("switch", { name: `${title} 품절`, exact: true })).toHaveAttribute("aria-checked", "false");
      await page.locator(".events-admin-list > li").filter({ hasText: title }).getByRole("button", { name: "수정", exact: true }).click();
      await page.getByLabel("구매하기 링크 (선택)", { exact: true }).fill("");
      await page.getByRole("button", { name: "저장", exact: true }).click();
      await expect(page.getByText("저장했습니다.", { exact: true })).toBeVisible();
      await page.goto(`/ko/${path}/${slug}`);
      await expect(page.getByRole("link", { name: "구매하기 (새 창)", exact: true })).toHaveCount(0);
