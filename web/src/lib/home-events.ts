...
export type HomeEvent = Pick<EventRecord, "registrationClosed" | "id" | "slug" | "title" | "titleEn" | "date" | "time" | "venueType" | "location" | "locationEn" | "image" | "link" | "ticketPriceKrw" | "isOnline"> & {
  readonly paymentHref?: string | undefined;
};

export function homeEvents(records: readonly EventRecord[]): HomeEvent[] {
  return records.map(({ registrationClosed, id, slug, title, titleEn, date, time, venueType, location, locationEn, image, images, link, ticketPriceKrw, isOnline }) => ({
    registrationClosed, id, slug, title, titleEn, date: date.trim().replaceAll(".", "-"), time, venueType, location, locationEn, link, ticketPriceKrw, isOnline,
