...
import { trustedUrl } from "./transport";
...
export async function createLnurlInvoice({ payment, receiver, transport }: ProviderContext): Promise<Invoice> {
  ...
  if (!name || !domain || !/^[A-Za-z0-9._-]+$/.test(name)) throw new PaymentError("INVALID_RECEIVER");
  const resolveUrl = trustedUrl(`https://${domain}/.well-known/lnurlp/${encodeURIComponent(name)}`, receiver.allowedOrigins);
  ...
  const verifyUrl = trustedUrl(result.verify, receiver.allowedOrigins).href;
...
export async function readLnurlInvoice({ payment, receiver, transport }: ProviderContext): Promise<Observation> {
  ...
  const result = verifyResponse.parse(await transport({ url: trustedUrl(payment.verifyUrl, receiver.allowedOrigins).href }));
