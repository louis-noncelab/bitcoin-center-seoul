...
import { CollectionFrame, ShopPurchaseLink, collectionCopy, collectionMetadata, collectionText } from "@/components/site/collection-public";
...
const readItem = cache(async (value: string) => {
  await connection();
  const item = await getCollectionByPath(value, false, libraryKinds);
  if (!item) notFound();
  return item;
});
...
export default async function CollectionDetailPage({ params }: Props) {
  ...
  return <CollectionFrame locale={locale} section="library" title={content.title} detail><article className="event-detail">
    ...
    {isPurchasableKind(item.kind) && <div className="button-row"><ShopPurchaseLink locale={locale} /></div>}
