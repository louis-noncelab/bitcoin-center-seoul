...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const paths = [
    ...
    ...listCollection(false, libraryKinds).map((item) => `/collection/${item.slug || item.id}`),
    ...listCollection(false, ["boardgame"]).map((item) => `/experience/board-game/${item.slug || item.id}`),
