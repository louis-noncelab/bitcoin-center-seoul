...
export function ReviewsSelection({ records, value, disabled, dirty, onChange, onSave }: {
  ...
  return <form className="events-form" onSubmit={(event) => { event.preventDefault(); onSave(); }}>
    ...
    <fieldset className="events-editor-fields events-field-grid" disabled={disabled}>
      <label>후기 페이지 대표<FormControl><MenuSelect value={value.featured_id ?? ""} onChange={(event) => onChange({ ...value, featured_id: event.target.value ? Number(event.target.value) : null })}>{options(value.featured_id, true)}</MenuSelect></FormControl></label>
      {[0, 1, 2].map((index) => <label key={index}>홈 후기 {index + 1}<FormControl><MenuSelect disabled={index > value.home_ids.length} value={value.home_ids[index] ?? ""} onChange={(event) => {
      ...
      }}>{options(value.home_ids[index] ?? null, false)}</MenuSelect></FormControl></label>)}
    </fieldset>
