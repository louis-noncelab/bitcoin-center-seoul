...
export function DateField({ name, label, defaultValue = "", required = false, onDirty }: {
  ...
  return <div className="date-field">
    ...
    <div className="date-field-control">
      ...
      <Button variant="quiet" className="date-field-trigger" aria-label={`${label} 달력 열기`} aria-haspopup="dialog" aria-expanded={open} onClick={() => { setToday(new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Seoul" }).format(new Date())); setOpen(true); }}><CalendarDays className="icon" aria-hidden="true" /></Button>
    </div>
    ...
    <Dialog open={open} onClose={() => setOpen(false)} title={`${label} 선택`} className="date-field-dialog">
      {open && <Calendar locale="ko" today={today} {...(isCalendarDate(value) ? { selected: value } : {})} minMonth="0100-01" maxMonth="9999-12" onSelect={choose} />}
