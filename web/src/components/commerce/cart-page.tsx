...
import { clearCart, useCartHydrated, useCartItems } from "./cart-store";
...
export function CartPageClient({ locale }: { readonly locale: Locale }) {
  ...
  const blocked = resolved.missing.length > 0 || resolved.lines.some((line) => !line.available) || ready.length === 0;
  return <div className="commerce-checkout">
    <div className="form-stack">
      <CartItemRows locale={locale} lines={resolved.lines} missing={resolved.missing} />
      {confirmClear
