...
export function PaymentInvoice({ invoice, checkoutUrl, locale }: { readonly invoice: string | null; readonly checkoutUrl: string | null; readonly locale: Locale }) {
  ...
  return <div className="form-stack commerce-invoice">
    {bolt11 && <>
      ...
      <details className="commerce-disclosure" open={copy === "failed"}><summary>{ko ? "결제 요청 보기" : "View payment request"}</summary><label className="sr-only" htmlFor="bolt11">{ko ? "라이트닝 결제 요청" : "Lightning payment request"}</label><textarea id="bolt11" className="commerce-bolt11" readOnly value={bolt11} onFocus={(event) => event.currentTarget.select()} /></details>
    </>}
