...
export function ShippingFields({ locale, fulfillment, country, countries, onCountry, error, draft }: {
  ...
  return <fieldset className="commerce-fieldset form-stack" hidden={fulfillment === "PICKUP"} disabled={fulfillment === "PICKUP"}>
    ...
    <FormField id="shipping-country" label={ko ? "국가" : "Country"}>
      <select id="shipping-country" autoComplete="shipping country" value={country} required aria-invalid={Boolean(countryError)} aria-describedby={countryError ? "shipping-country-error" : undefined} onChange={(event) => onCountry(event.target.value)}>
    ...
    <FormField id="shipping-line1" label={ko ? "주소" : "Street address"}>
      <input aria-invalid={Boolean(line1Error)} aria-describedby={line1Error ? "shipping-line1-error" : undefined} id="shipping-line1" name="line1" autoComplete="shipping address-line1" maxLength={200} required defaultValue={draft?.line1 ?? ""} />
      <FieldError id="shipping-line1" error={line1Error} />
    </FormField>
    <FormField id="shipping-line2" label={ko ? "상세주소 (선택)" : "Apartment, suite, etc. (optional)"}>
      <input aria-invalid={Boolean(line2Error)} aria-describedby={line2Error ? "shipping-line2-error" : undefined} id="shipping-line2" name="line2" autoComplete="shipping address-line2" maxLength={200} defaultValue={draft?.line2 ?? ""} />
      <FieldError id="shipping-line2" error={line2Error} />
    </FormField>
    <div className="form-row">
      <FormField id="shipping-city" label={ko ? "시·군·구" : "City"}>
        <input aria-invalid={Boolean(cityError)} aria-describedby={cityError ? "shipping-city-error" : undefined} id="shipping-city" name="city" autoComplete="shipping address-level2" maxLength={100} required defaultValue={draft?.city ?? ""} />
        <FieldError id="shipping-city" error={cityError} />
      </FormField>
      <FormField id="shipping-region" label={ko ? "시·도 / 주 (선택)" : "State / province (optional)"}>
        <input aria-invalid={Boolean(regionError)} aria-describedby={regionError ? "shipping-region-error" : undefined} id="shipping-region" name="region" autoComplete="shipping address-level1" maxLength={100} defaultValue={draft?.region ?? ""} />
