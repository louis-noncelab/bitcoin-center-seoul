...
## Journal navigation correction — 2026-09-09

The owner rejected scrolling the outgoing listing to the top before following a card or pagination link. `ContentLink` now starts native Next.js navigation directly, retaining `prefetch={false}` and the existing destination opacity fade. Next positions the destination after routing; native back/forward navigation restores the reading position. This supersedes the earlier scroll-before-navigation description. The explicit floating back-to-top action is unchanged.

The corrected regression failed twice on the old build: normal motion produced intermediate scroll positions under the old URL, and reduced motion reset it to zero before routing. After removing the custom animation/router interception, production build, TypeScript, ESLint and all18 navigation/SEO checks passed. The added journal history check verifies returning to the lower card's reading position. Evidence is in `web/.local/navigation-transition/{red,check,build,green}.log`.

Root's real Chrome check also delayed a page-two response by250ms: the old page remained still until routing, page two opened at the top with the correct canonical, middle-click opened a new tab without moving the source page, keyboard navigation and history worked, and sibling program/notice links worked. No page errors were observed. Evidence: `manual.json` and `manual-page2.png` in the same directory. Fresh responsive/motion evidence and independent reviews are retained there; their final verdicts bind to the recorded source/build identity. No authentication behavior or operational configuration changed in this increment.
